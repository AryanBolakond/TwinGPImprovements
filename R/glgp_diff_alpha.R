    ...
    for (j in i:gNum_) {
      if (i == j) {
        ...
        for (d in seq_len(dim_)) {
          ...
          value <- value - gParams_[d] * temp^gParams_[dim_ + d]
          dist <- dist + temp^2
        }
  ...
  for (i in seq_len(gNum_)) {
    for (j in i:gNum_) {
      if (i == j) {
        ...
        for (d in seq_len(dim_)) {
          temp <- abs(row_i[d] - row_j[d])
          value <- value - gParams[d] * temp^gParams[dim_ + d]
        }
        Rg[i, j] <- exp(value)
        Rg[j, i] <- Rg[i, j]
      }
...
estimate_gParams <- function(gp) {
  ...
  lb <- c(rep(1e-7, dim_), rep(1.0, dim_), log(1e-7))
  ub <- c(rep(1000.0, dim_), rep(2.0, dim_), 0.0)
  
  opt_dim <- if (nugget_) 2L * dim_ + 1L else 2L * dim_
  nugget_init <- if (nugget_) log(1e-3) else log(1e-7)
  
  rho <- 1.0 / sqrt(rep(dim_, dim_))
  scale_grid <- exp(seq(3.0, -3.0, length.out = 11L))
  
  nllg_values <- vapply(scale_grid, function(a) {
    gParams <- numeric(2L * dim_ + 1L)
    gParams[seq_len(dim_)] <- a * rho
    gParams[dim_ + seq_len(dim_)] <- 1.95
  ...
  for (i in seq_len(num_opt)) {
    gParams <- numeric(2L * dim_ + 1L)
    gParams[seq_len(dim_)] <- scale_grid[min_index] * rho * factor[i]
    gParams[dim_ + seq_len(dim_)] <- 1.95
    gParams[2L * dim_ + 1L] <- nugget_init
...
estimate_sParams <- function(gp) {
  ...
  gp$nug_ <- (1.0 - gp$lam_) * gp$gParams_[2L * dim_ + 1L] + gp$lam_ * exp(sParams[2L])
  invisible(gp)
}
...
#'
#' Uses a per-dimension correlation exponent (Option b): gParams_ has
#' dimension 2*d+1, laid out as [theta_1..theta_d, alpha_1..alpha_d, nugget],
#' vs. the single shared alpha (dimension d+2) used in glgp.R.
